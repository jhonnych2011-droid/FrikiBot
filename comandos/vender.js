...
export async function run(sock, msg, args, geosDB) {
  ...
  if (!args.length) return sock.sendMessage(from, { text: "⚠️ Uso: .vender <nombre del personaje>" }, { quoted: msg });
  ...
  if (inventario[sender].length === 0) return sock.sendMessage(from, { text: "❌ No tienes personajes para vender." }, { quoted: msg });

  const index = inventario[sender].findIndex(n => n.toLowerCase() === nombreBuscado);
  if (index === -1) return sock.sendMessage(from, { text: "❌ No tienes ese personaje en tu inventario." }, { quoted: msg });

  const personajeNombre = inventario[sender][index];

  if (!personajes[personajeNombre]) return sock.sendMessage(from, { text: `❌ El personaje *${personajeNombre}* no existe.` }, { quoted: msg });

  const precioOriginal = personajes[personajeNombre].precio;
  const ganancia = Math.floor(precioOriginal * 0.96);

  // Sumar geos correctamente
  ...
  // Actualizar stock
  personajes[personajeNombre].stock = (personajes[personajeNombre].stock || 0) + 1;
  guardarArchivo(personajesFile, personajes);

  return sock.sendMessage(from, { text: `💰 Vendiste *${personajeNombre}* y recibiste *${ganancia} geos*.\n📦 Stock actual: ${personajes[personajeNombre].stock}` }, { quoted: msg });
}
